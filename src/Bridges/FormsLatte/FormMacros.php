...
class FormMacros extends MacroSet
{
	...
	public static function install(Latte\Compiler $compiler)
	{
		...
		$me->addMacro('form', array($me, 'macroForm'), 'echo Nette\Bridges\FormsLatte\FormMacros::renderFormEnd($_form)');
	...
	public function macroForm(MacroNode $node, PhpWriter $writer)
	{
		...
		return $writer->write(
			'echo Nette\Bridges\FormsLatte\FormMacros::renderFormBegin($form = $_form = '
	...
	public function macroNameEnd(MacroNode $node, PhpWriter $writer)
	{
		...
		if (strtolower($node->htmlNode->name) === 'form') {
			$node->content = $parts[1] . $parts[2] . '<?php echo Nette\Bridges\FormsLatte\FormMacros::renderFormEnd($_form, FALSE) ?>' . $parts[3];
